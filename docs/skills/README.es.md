...
Una colección de 58 skills para Claude Code que codifican ingeniería de software disciplinada, razonamiento forense y construcción orientada a la seguridad para el pipeline local de DFIR y detección de ZAYNOR. Cada skill se activa automáticamente cuando la conversación coincide con sus condiciones de trigger, inyectando metodología sin que el usuario tenga que pedirla.
...
| ID de origen | Skill | Categoría | Se activa cuando |
|---:|---|---|---|
...
Este catálogo excluye intencionalmente entrenamiento y evaluación de machine
learning, cloud, OT, cliente, bases de datos, concurrencia genérica, backups,
adquisición, ingeniería social y prácticas específicas de deception. Esos temas
pueden ser útiles en otros proyectos, pero no forman parte del slice local de
DFIR y detección actual de ZAYNOR. El catálogo puede crecer cuando la
implementación agregue un contrato concreto que necesite alguno de ellos.

---

## Estructura del repositorio

Cada skill vive en su propio directorio y contiene un archivo `SKILL.md` autocontenido:
...
Algunas skills también incluyen subdirectorios `scripts/` o `references/` cuando
los helpers ejecutables o el material de referencia largo no corresponden al
cuerpo siempre cargado.
